...
// Маскированный орк — wanderer с агрессивным переходом в погоню и большой
// дальностью обнаружения. HP=2. Похож на Wanderer, но «злее».
export class MaskedOrc extends Monster {
  ...
  canSee(player, map) {
    ...
    if (Math.hypot(dx, dy) > WANDERER_VISION_TILES + 2) return false;
  ...
  update(dtMs, player, map) {
    ...
    if (this.canSee(player, map)) {
      this.mode = 'chase';
      this.loseInterestAt = now + WANDERER_LOSE_INTEREST_MS + 1500;
